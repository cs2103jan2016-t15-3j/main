...
//    private final int VIEW_INDEX = 5;
    ...
    public Command parse(String input) {
        ...
        if (type == null) {
            return new InvalidCommand("View type not detected");
        }
        switch (type) {
            case DESCRIPTION:
                if (parameter == null) {
            ...
            case FROM:
                ...
                if (timeParserResult.getMatchString() == null || !timeParserResult.getMatchString().equals(parameter)) {
                    return new InvalidCommand("Invalid Time");
                }
                if (timeParserResult.getFirstDate() != null && timeParserResult.getSecondDate() != null) {
                    if (timeParserResult.getFirstTime() == null && timeParserResult.getSecondTime() == null) {
                        result.setStartDate(timeParserResult.getFirstDate());
                        result.setEndDate(timeParserResult.getSecondDate());
                    }
                    else if (timeParserResult.getFirstTime() != null && timeParserResult.getSecondTime() != null) {
                        result.setStartDate(timeParserResult.getFirstDate());
                        result.setEndDate(timeParserResult.getSecondDate());
